"""
================================================================================
HAL 9000: VECTOR BATTLEZONE v2.4 (Forward Kinematics & Modular Engine)
...
class Tank:
    def __init__(self, x, z):
        ...
        # HAL UPGRADE: Forward Kinematics Physics State
        self.sway = 0.0
        self.sway_vel = 0.0
    ...
    def update(self, px, pz):
        ...
        # --- Antenna Spring-Mass Damper Physics ---
        # 1. Inertia: Turn pushes the antenna the opposite way
        self.sway_vel -= actual_turn * 0.8  
        # 2. Rumble: Simulate rough terrain while driving
        self.sway_vel += random.uniform(-0.02, 0.02)
        # 3. Hooke's Law: Pull back to center 
        self.sway_vel -= self.sway * 0.15 
        # 4. Damping: Friction to stop infinite oscillation
        self.sway_vel *= 0.85             
        # Apply to angle
    ...
    def draw(self, oled, cx, cy, cz, cam_sin, cam_cos):
        ...
        # Draw Dynamic Segmented Whip Antenna
        ax, ay, az = 0.0, 5.5, -5.0 # Local start point (top rear of hull)
        seg_len = 3.5 # 3 segments = 10.5 total height (roughly 2x tank height)
        
        for i in range(1, 4):
            # The bend becomes more severe higher up the antenna
            bend = self.sway * i 
            
            # Calculate next local joint based on bend angle
            nx = ax + math.sin(bend) * seg_len
            ny = ay + math.cos(bend) * seg_len
            nz = az # Stays flat relative to local Z plane
            
            # Transform Segment Start to World Space
            wx1 = ax * tcos + az * tsin + self.x
            wz1 = -ax * tsin + az * tcos + self.z
            
            # Transform Segment End to World Space
...
def run_battlezone():
    print("HAL 9000: Initializing Simulation v2.4 (Kinematic Antennas Active)")
