"""
================================================================================
HAL 9000: VECTOR BATTLEZONE v2.1 (Centered Cinematic & Shimmer)
...
# --- THE OLED DRIVER (V2: Snapshot Capability) ---
class FastOLED_128x32:
    ...
    def pixel(self, x, y, color=1):
        ...
        if 0 <= x < 128 and 0 <= y < 32:
            idx = x + (y // 8) * 128
            if color: self.buffer[idx] |= (1 << (y % 8))
            else: self.buffer[idx] &= ~(1 << (y % 8))
...
# ==============================================================================
# MAIN DEMO: 3D VECTOR BATTLEZONE v2.1
# ==============================================================================
...
# HAL UPGRADE: Added 'scx' (Screen Center X) to decouple viewport centering from game math
def draw_3d_line(oled, x1, y1, z1, x2, y2, z2, cx, cy, cz, cam_sin, cam_cos, cl_xmax=99, scx=50):
    ...
    FOV = 60
    # Center X is now dynamic based on the scx parameter
    ...
    if clipped_screen:
        oled.line(clipped_screen[0], clipped_screen[1], clipped_screen[2], clipped_screen[3])
...
def draw_3d_point(oled, x, y, z, cx, cy, cz, cam_sin, cam_cos):
    ...
    if rz > 0.1:
        ...
        if 0 <= sx < 99 and 0 <= sy < 32: oled.pixel(sx, sy)
...
class Tank:
    ...
    def update(self, px, pz):
        ...
        if dx**2 + dz**2 < 20000: 
            ...
            if abs(yaw_diff) > 0.1: self.yaw += 0.04 if yaw_diff > 0 else -0.04
            else:
                self.cooldown -= 1
                if self.cooldown <= 0:
                    self.cooldown = 45 
...
def generate_cracks():
    cracks = []
    # Expanded targets to ensure a uniform distribution across the entire 128px width
    targets = [(0,0), (127,0), (0,31), (127,31), (64,0), (64,31), (0,16), (127,16), (32,0), (96,0), (32,31), (96,31)]
    # Absolute center of the OLED
    cx, cy = 64, 16 
...
def run_battlezone():
    print("HAL 9000: Initializing Simulation v2.1")
    ...
    debug_fps, frames, current_fps, last_time = True, 0, 0, time.ticks_ms()
    gc.collect() 
    ...
    try:
        while True:
            ...
            # --- Game State 3: Shimmering Text Overlay ---
            if game_state == 3:
                if pad[6]:
                ...
                # HAL FIX: The Shimmer
                # Oscillates smoothly between Z=27 and Z=33. 
                # At Z=30, the text is exactly 120 pixels wide, fitting perfectly inside the 128px screen.
                tz = 30 + math.sin(frames * 0.3) * 3.0
                
                # Render using the Neutral Camera (0,0,0) and forcing screen center to 64
                for lx1, ly1, lz1, lx2, ly2, lz2 in game_over_mesh:
                    draw_3d_line(oled, lx1, ly1, lz1 + tz, lx2, ly2, lz2 + tz, 
                                 0, 0, 0, 0.0, 1.0, cl_xmax=127, scx=64)
                oled.show()
                continue
            ...
            if game_state == 0:
                ...
                if pad[4] and len(player_missiles) < 3 and player_fire_cooldown == 0:
                    player_missiles.append([px, py-0.5, pz, pyaw, 25]) 
            ...
            if game_state == 0:
                ...
                for em in enemy_missiles[:]:
                    ...
                    if (em[0] - px)**2 + (em[2] - pz)**2 < 16:
                        ...
                        oled.rect(100, 0, 28, 32, color=0, filled=True)
                        oled.vline(99, 0, 32, color=1) 
                        radar_cx, radar_cy = 114, 16
                        oled.pixel(radar_cx, radar_cy, color=1) 
                        R_SCALE = 0.05
                        for obj in static_objects:
                            ...
                            if 101 <= sx <= 126 and 1 <= sy <= 30: oled.pixel(sx, sy, color=1)
                        for t in tanks:
                            rx = (t.x - px) * cam_cos - (t.z - pz) * cam_sin; rz = (t.x - px) * cam_sin + (t.z - pz) * cam_cos
                            sx, sy = int(radar_cx + rx * R_SCALE), int(radar_cy - rz * R_SCALE)
                            if 101 <= sx <= 126 and 1 <= sy <= 30: oled.rect(sx-1, sy-1, 3, 3, color=1, filled=True)
            ...
            if game_state == 0:
                ...
                for i in range(crack_limit):
                    ...
                    if clipped: oled.line(clipped[0], clipped[1], clipped[2], clipped[3])
                oled.show()
                ...
                # HAL FIX: Swoop inward to Z=30 for maximum screen real estate
                tz, ty = 150 - (120 * t), -math.sin(t * math.pi) * 8.0 
                
                # Render using the Neutral Camera (0,0,0) and forcing screen center to 64
                for lx1, ly1, lz1, lx2, ly2, lz2 in game_over_mesh:
                    draw_3d_line(oled, lx1, ly1 + ty, lz1 + tz, lx2, ly2 + ty, lz2 + tz, 
                                 0, 0, 0, 0.0, 1.0, cl_xmax=127, scx=64)
                oled.show()
