"""
================================================================================
HAL 9000: VECTOR BATTLEZONE v2.3 (Cylindrical Collision & Modular Engine)
Hardware: RP2350 (Pico 2) | I2C1: SDA=GP14, SCL=GP15
Requires: hal_engine.py
================================================================================
"""
import machine
import time
...
def run_battlezone():
    print("HAL 9000: Initializing Simulation v2.3 (Collisions Active)")
    
    # Init Hardware via External Module
    ...
    try:
        while True:
            ...
            if game_state == 0:
                ...
                # HAL UPGRADE: Cylindrical Collision Detection
                # Prevents the player from driving through the static geometry
                for obj in static_objects:
                    ox, oz = obj[0], obj[1]
                    dx, dz = px - ox, pz - oz
                    dist_sq = dx**2 + dz**2
                    
                    # 256 is 16 squared. A radius of 16 perfectly envelopes the static models.
                    if dist_sq < 256: 
                        dist = math.sqrt(dist_sq)
                        if dist == 0: dist = 0.1 # Prevent div by zero if exact center
                        
                        # Calculate the penetration depth
                        overlap = 16.0 - dist
                        
                        # Push the player outward along the collision normal
                        px += (dx / dist) * overlap
                        pz += (dz / dist) * overlap
